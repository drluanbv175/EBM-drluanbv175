...
> luật thường trực (đề xuất #12 của `audit/12-danh-gia-hoan-thien-he-thong_2026-09-21.md`).
> Giữ NGUYÊN VĂN để tra lịch sử: mọi mã BH, số đo theo ngày, các khối «ĐÍNH CHÍNH».
> Tra bằng `grep -n "<từ khoá>" audit/NHAT-KY-SU-CO.md` — không đọc cả tệp (~100k token).
>
> **Sự cố / bài học MỚI từ 24/09/2026:** ghi vào mục «SAU 24/09/2026» ở cuối tệp này
> (ngày · mã BH nếu có · nguyên nhân · cách vá · kiểm hồi quy). CHỈ đưa vào `CLAUDE.md`
> khi nó sinh ra một LUẬT THƯỜNG TRỰC mới, và viết luật đó gọn trong 1–3 dòng.
>
> ⚠️ Số đo trong phần lưu trữ là ẢNH CHỤP theo ngày ghi — không trích lại làm hiện trạng.
...
- **✅ SKILL DÙNG NGUỒN CHUNG VÀ TỰ ĐỒNG BỘ (hoàn thiện 2026-08-20).**
  ...
  **hai file CỘNG DỒN** (cả hai đều chạy), nên không mất chốt nào. `dong_bo_hook_sessionstart.py` phạm
  ...
  🔴 **NGUYÊN NHÂN GỐC vì sao hook cloud KHÔNG tự bắn khi phiên có ≥2 REPO — đo 09/09/2026 (BH103;
  đánh số lại 16/09/2026 — "BH99" đã bị hai lỗi khác chiếm trước khi lô này cherry-pick vào, xem
  BH99/BH100/BH101/BH102 ở trên).**
  ...
  **Biện pháp đang áp dụng, nói rõ giới hạn từng cái — KHÔNG cái nào là "đã đóng gốc":**
  ...
  khi kịp làm) vẫn có thể lọt — **BH103 chỉ kiểm được chữ chỉ thị này CÓ MẶT trong file tracked,
  ...
  ra chính lỗi BH103 ở phiên đa-repo, nhưng phiên hiện tại **không có công cụ đọc lại nhật ký
...
## SAU 24/09/2026 — sự cố & bài học mới (ghi tiếp từ đây)

### 24/09/2026 — Xoá bản sao « 2» của OneDrive làm mất luôn bản gốc
Bốn thư mục bản sao OneDrive (`quality/eval/tra-diem-kham 2`, `sync/scheduled-tasks/{giam-sat-acc-aha-quy,
kiem-thang-diem-quy,kiem-tra-hoan-thien-he-thong-thang} 2`) được so `diff -rq` giống hệt bản gốc rồi
đưa vào Thùng rác. Vài chục giây sau (trong lúc OneDrive đồng bộ), **bốn thư mục GỐC cũng biến mất**
khỏi cây làm việc (`git status` báo ` D`). Khôi phục bằng `git checkout -- <đường dẫn>`; so `cmp` với bản
trong Thùng rác: giống hệt. Nhiều khả năng OneDrive coi hai mục cục bộ là cùng một mục trên mây, nên
xoá một bản là xoá mục trên mây rồi lan xuống bản còn lại.
**Luật rút ra:** không xoá bản sao « 2» khi bản gốc đang được git theo dõi mà không kiểm lại; dọn xong
phải đợi OneDrive đồng bộ, chạy `git status` và khôi phục bằng `git checkout --` nếu bản gốc mất.
Giữ bản trong Thùng rác cho tới khi xác nhận.

### 24/09/2026 — CLAUDE.md rút về bản gọn
Toàn văn cũ (278.003 ký tự, 26 khối «ĐÍNH CHÍNH») chuyển nguyên văn vào tệp này. `CLAUDE.md` mới chỉ
giữ luật thường trực + con trỏ; `tools/verify_claude_code_repo_alignment.py` chặn commit khi
`CLAUDE.md` vượt ngân sách ký tự (giữ gọn lâu dài). Ba chốt đọc nội dung `CLAUDE.md` (BH103,
`verify_claude_code_repo_alignment.py`, `verify_plugin_orchestration.py`) vẫn tìm thấy đủ chuỗi bắt buộc.

### 24/09/2026 — Loại xuất bản PubMed `consensus development conference` đã bị khai tử: 42/47 chủ đề quét «suy giảm»
Kỳ quét tuần 21/09 (W39) báo `PARTIAL` — 42/47 chủ đề suy giảm, 1 FAIL (RA) — cùng thông điệp
`NCBI không nhận ra thẻ/cụm trong truy vấn: ['conference']`. Bản W39 nghi bộ quét (luật `errorlist/phrasesnotfound`
thêm 21/09) «nâng cảnh báo vô hại thành lỗi». **Đo sống qua connector PubMed ngày 24/09 cho thấy ngược lại:**
`consensus development conference[pt]` và `"consensus development conference"[pt]` đều **0 kết quả, PubMed không dịch
được**; `practice guideline[pt]` (116 bài cho bệnh thần kinh do ĐTĐ), `guideline[pt]` (43.851), `"consensus
statement"[pt]` (29.932) và `"consensus development conference, nih"[pt]` (781) đều được dịch và có kết quả. Tức NLM đã
bỏ loại xuất bản này; luật mới của bộ quét báo ĐÚNG — trước 21/09 cụm này âm thầm không khớp gì (tầng guideline mất
phần consensus mà không ai biết).
**Đã sửa:** 42 truy vấn tầng guideline trong `EBM-Dashboards/watchlist.json` (sao lưu
`watchlist.json.bak-20260924-truoc-sua-pt`) và 2 truy vấn trong `tools/nap_guideline_pmc.py` → `"consensus statement"[pt]`.
...
### 26/09/2026 (khuya, tiếp 2) — quét `upgrade_verify.py` trên Cloud: 12/28 bước ❌
- **Lỗi thật (mọi máy):** bước 10 `verify_research_practical_readiness.py` tự mâu thuẫn — bản vá 17/09 đổi `visit_date` sang "text" (ngày gắn với cá nhân là PHI ⇒ CỐ Ý redact) và thêm kiểm «phải redact», nhưng không gỡ khối 08/09 kiểm «được miễn, GIỮ nguyên» ⇒ FAIL vĩnh viễn; bước 12 đỏ lây. Sửa: luồng "text" kiểm nhất quán (không miễn); cơ chế miễn cho cột khai "date" (chỗ NỚI luật, không test nào khác canh) được giữ bằng `_verify_declared_date_exemption()` với dictionary riêng. Bài học: bản vá đổi hành vi phải gỡ luôn assertion của hành vi cũ — hai assertion ngược chiều trong một tệp là dấu hiệu chắc chắn.
- **Lỗi đường dẫn (Cloud):** 4 verifier repo y khoa viết cứng `ROOT = REPO.parent` ⇒ bố cục anh em tìm `sync/skills/` ở `/home/user` (repo y khoa, `tools/goc_ebm.py`).
- **Không đo được (bản sao trần):** sổ công cụ orchestrator đòi `EBM-Dashboards/tools/verify_dashboard.py` ⇒ nay ⚪ + dòng «không đo được» trong `--validate`. Còn lại (13/14/19/23/24, phần tệp OneDrive của 21/22) đều thiếu thư mục chỉ-OneDrive; bước 28 = `core.hooksPath` chưa bật trong container.
