...
Note: this offline VN adaptation does not include AI-based figure/image generation (which required a paid third-party API key, and the original text here mandated 3-30 AI-generated images per document). For diagrams, use the markdown-mermaid-writing skill instead (free, offline).
...
appear in the title — repeating title words adds no discoverability. For medical
papers, check each term against the MeSH Browser; mark unverified terms as
`[CẦN KIỂM CHỨNG NGUỒN CHÍNH THỨC]` rather than guessing.
...
> ⚕️ **SỬA 06/09/2026 — quy tắc cũ SAI cho y khoa.** Bản gốc viết "NEVER use labeled
> sections", trong khi chính file này ở mục *Venue-Specific Writing Styles* nói tạp chí
> y khoa (NEJM, Lancet) dùng **structured abstracts**. Bác sĩ dùng skill này để nộp tạp
> chí y khoa, nên mặc định phải ngược lại.

- **Y khoa mặc định CÓ CẤU TRÚC.** Dùng nhãn mục (Background/Objective · Methods ·
  Results · Conclusions) trừ khi hướng dẫn tác giả của tạp chí đích nói ngược lại.
  **Kiểm hướng dẫn tác giả TRƯỚC khi viết** — đừng đoán.
- Chỉ viết văn xuôi liền mạch không nhãn khi tạp chí yêu cầu vậy (một số tạp chí khoa
  học cơ bản), hoặc khi đó là abstract hội nghị có quy định riêng.
- Đủ **5 phần** dù có nhãn hay không: bối cảnh · mục tiêu · phương pháp · kết quả (kèm
  ước lượng và **95% CI**, không chỉ p) · kết luận.
