...
description: 'Tính CỠ MẪU / LỰC THỐNG KÊ (power analysis) TRƯỚC khi thu thập dữ liệu — bằng công thức đóng (t-test, ANOVA, tỷ lệ, tương quan, chi-square, hồi quy) VÀ mô phỏng Monte Carlo cho thiết kế không có công thức chuẩn (hồi quy logistic/Poisson, mixed model, cluster-RCT, sống còn, có tương tác). Dùng khi cần trả lời ''cần bao nhiêu bệnh nhân'', biện minh cỡ mẫu cho đề cương/IRB, hoặc vẽ đường cong power. Bổ sung công cụ tính chạy được cho agent co-mau-nghien-cuu (G3) — chạy offline bằng Python, không cần API.'
