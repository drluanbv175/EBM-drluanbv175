...
description: 'Phân tích SỐNG CÒN / thời gian-đến-biến-cố (time-to-event) bằng scikit-survival: mô hình Cox, Random Survival Forest, Gradient Boosting, Survival SVM; xử lý dữ liệu KIỂM DUYỆT (censoring) và NGUY CƠ CẠNH TRANH (competing risks); đánh giá bằng c-index/Brier score. Dùng khi kết cục nghiên cứu là thời gian tới biến cố (tử vong, tái phát, biến chứng...). Bổ sung cho phan-tich-thong-ke (G6) — chạy offline bằng Python, không cần API.'
