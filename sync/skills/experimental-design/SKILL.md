...
description: 'Thiết kế thí nghiệm/nghiên cứu TRƯỚC khi thu thập dữ liệu — chọn thiết kế, ngẫu nhiên hóa (randomization), phân khối (blocking/stratification), bố trí factorial/fractional-factorial, crossover, split-plot, Latin square, kiểm soát hiệu ứng thứ tự/đợt chạy. Dùng khi cần phân nhóm bệnh nhân/mẫu vào các nhánh, tránh nhiễu (confounding), hoặc lập kế hoạch thử nghiệm nhiều yếu tố. Bổ sung công cụ sinh sơ đồ phân nhóm chạy được cho thiet-ke-nghien-cuu (G1) — chạy offline bằng Python, không cần API.'
