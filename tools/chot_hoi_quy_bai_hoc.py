...
def _goc_dash() -> Path | None:
    ...
    VÁ 09/09/2026: BH30/BH39 tự ghép cứng `REPO / "EBM-Dashboards"` — cùng lỗi
    BH #105 đã vá cho `_goc_mea()`. Trên máy THẬT (kiến trúc OneDrive: cây gốc
    "Claude AI" vừa là repo git vừa chứa `EBM-Dashboards/` làm thư mục con
    không track) ghép cứng vẫn đúng; trên phiên cloud (`add_repo` dựng các repo
    làm ANH EM dưới cùng thư mục cha) nó luôn rỗng dù không liên quan gì tới
    nội dung đang kiểm. `duong_goc()` đã tự kiểm cả hai vị trí nên dùng nó thay
    vì tự đoán một vị trí. Trả None khi thật sự vắng ở cả hai — gọi nơi dùng
    phải tự xử lý, không giả định luôn có giá trị.
    """
...
def bh07_doc_secrets_ngoai_onedrive():
    ...
    """
    t = (_goc_mea() / "app" / "config.py").read_text(encoding="utf-8", errors="replace")
...
def bh39_doctrine_khong_duoc_troi_sau_cong():
    ...
    *"bài có bị rút không?"* mà KHÔNG đưa công cụ nào — tức bảo mô hình trả lời bằng trí
    ...
    """
    # VÁ 09/09/2026: bản đầu chỉ tìm bản doctrine-canonical (REPO/EBM-Dashboards/tools/…),
    # không lùi về bản vendor-qua-git — cùng khoảng trống mà duong_cong_cu_pipeline()
    # (tools/ban_sao_tran.py) đã lấp cho xuat_goi_cap_nhat.py/canary 08/09/2026, chỉ là
    # BH39 chưa từng được nối dây theo. Dùng resolver chung: máy thật vẫn thấy bản
    # canonical như cũ; phiên cloud/CI lùi về sync/skills/cap-nhat-chung-cu-y-khoa/tools/
    # (LUÔN có qua git) — cổng ĐƯỢC KIỂM THẬT thay vì chỉ ⚪ "không thấy".
...
def bh56_cong_cu_moi_phai_co_day():
    ...
    noi_tieu_thu = [goc / ".claude" / "agents" / "tra-cuu-chung-cu.md",
                    ...
                    # Nguồn ĐI QUA GIT của cùng nội dung — VÁ 08/09/2026: bản trước chỉ
                    # đọc bản cài-máy Path.home() (không track git, luôn vắng trên clone
                    # mới/cloud) nên báo "MỒ CÔI" giả dù do_tac_dong ĐÃ được nhắc ở đây.
...
def phan_loai(ma: str, ok: bool, tran: bool, ct: str = "", bst=None) -> str:
    ...
    VÁ 08/09/2026 — «2-trong-3 gốc vắng» (phiên cloud: medical-ebm-automation có mặt
    dạng sibling, EBM-Dashboards/EBM_MASTER KHÔNG BAO GIỜ có trong bất kỳ git checkout
    nào) không được `tran` (đòi CẢ BA gốc vắng) nhận diện — đo thật: 30/34 mục "tái
    phát" trên phiên cloud chỉ vì `tran=False` toàn cục, dù đối tượng chúng soi (một
    file cụ thể dưới EBM-Dashboards/EBM_MASTER) THẬT SỰ vắng mặt. Nay: nếu thông điệp
    lỗi (`ct`, do chính `except BaseException` hoặc hàm bh*_ tự ghi) nêu ĐÍCH DANH một
    trong ba tên gốc, chỉ cần ĐÚNG gốc đó vắng là đủ ⚪ — không cần cả ba. `ct` rỗng
    hoặc không nêu tên gốc nào ⇒ lùi về `tran` toàn cục như cũ (KHÔNG bớt đường cũ,
    chỉ THÊM — cùng nguyên tắc `duong_goc()` đã áp ở tools/ban_sao_tran.py 07/09/2026).
    `bst` là module `ban_sao_tran` đã nạp sẵn (main() nạp MỘT lần); truyền None thì tự
    nạp khi cần — giữ mọi lời gọi trực tiếp (test) không đổi hành vi.
    """
    if ok:
        return "dat"
