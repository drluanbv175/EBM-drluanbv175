...
def build_criteria(deep: bool, py: str) -> List[Dict]:
    ...
    crit("S5", "Giám sát, guardrail, kiểm soát rủi ro", "system", [
        ...
        lambda: p_contains_all(
            TOOLS / "run_controlled_automation_cycle.py",
            ["FAIL_CLOSED", "CONTROLLED_READY_WITH_HUMAN_GATES", "clinical_production_allowed"],
            "Chu trình tự động có kiểm soát fail-closed/human-gated",
        ),
    ...
    crit("S6", "Đánh giá hiệu suất", "system", [
        ...
        lambda: p_contains_all(
            ...
            "Verifier hợp nhất thẩm định/phản biện/stakeholder/readiness/thống kê",
        ),
    ])
    return C
