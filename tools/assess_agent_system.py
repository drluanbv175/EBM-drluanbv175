...
CAVEATS: Dict[str, List[str]] = {
    "A2": ["Nhánh LÂM SÀNG (dieu-phoi-lam-sang 5 bước) là prose — chưa có "
           "orchestrator chạy được như run_pipeline của nghiên cứu (ngoài phạm vi "
           "vá A3/A6 2026-07-04 — chỉ suy luận lâm sàng đã có máy tính, chưa phải "
           "toàn bộ pipeline điều phối lâm sàng)."],
    "S2": ["Orchestrator điều phối SCRIPT cổng (run_g*_auto.py), không phải 48 agent "
           ".md trực tiếp; bàn giao agent↔agent do LLM lúc chạy quyết định, chưa mã "
           "hoá — NHƯNG nay có kiểm chứng bằng máy (verify_agent_routing.py) rằng "
           "mọi agent được nhạc trưởng THAM CHIẾU đúng, không tham chiếu treo."],
