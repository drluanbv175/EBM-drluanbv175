...
def test_readiness_report_unlocks_10_of_37_repo_controls_without_production():
    report = R.build_report()
    summary = report["summary"]

    assert summary["total_blockers"] == 37
    assert summary["repo_control_ready_unlocked"] == 10
    assert summary["expected_repo_control_unlocks"] == 10
...
def test_markdown_names_10_of_37_and_keeps_safety_boundary():
    markdown = R.markdown_report(R.build_report())

    assert "Repository-control unlocked: `10/37`" in markdown
