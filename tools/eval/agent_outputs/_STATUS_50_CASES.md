...
**Cập nhật 2026-07-05:** đã dựng sẵn 2 file chấm ĐẦY ĐỦ 50 ca (case_id + tóm tắt + khuyến nghị agent đã điền từ `cases_all_C01-C50.md`, chỉ còn cột chấm trống) — `templates/expert_kappa_50cases.csv` (50 dòng) + `templates/likert_50cases.csv` (150 dòng, 50 ca × 3 bác sĩ); đã vá `human_eval_score.py` (cột `summary`/`agent_rec` từng bị hiểu nhầm là nhãn chuyên gia → crash) và smoke-test cả 2 file đọc đúng. Việc DUY NHẤT còn lại, không thể tự làm:
1. Gửi `templates/expert_kappa_50cases.csv` cho **3 chuyên gia chấm mù** (điền `expert1/2/3`, KHÔNG đổi cấu trúc cột).
2. Gửi `templates/likert_50cases.csv` cho **3 bác sĩ** điền Likert (`c1..c5`).
3. Chạy `python3 human_eval_score.py --kappa templates/expert_kappa_50cases.csv --likert templates/likert_50cases.csv` → κ + Likert → cập nhật `_CHUAN-CAFES.md` P3.2/P4.2. (xem `templates/RUNBOOK.md`)
