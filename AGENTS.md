...
- **Đồng bộ skill/plugin tự động:** `sync/skills/` là nguồn duy nhất cho skill riêng. `tools/dong_bo_skill_claude_codex.py` tạo symlink sang `~/.claude/skills` và `~/.codex/skills`, cập nhật runtime Cowork có sao lưu, kiểm 9 plugin Claude→Codex, dựng lại catalog/ZIP `plugin-router-chatgpt`. Sửa nội dung skill có hiệu lực tức thời qua symlink; hook `SessionStart` tự bắt skill mới và thay đổi registry plugin. Không dùng LaunchAgent đọc OneDrive vì macOS TCC chặn tiến trình nền nếu chưa cấp Full Disk Access. Không sửa tay bản trong cache/runtime. Trong bộ hợp nhất, nguồn OneDrive thắng runtime Cowork nhưng bản cũ luôn được sao lưu; khi chạy `dong_bo_skill.py` riêng, mặc định vẫn chặn phân kỳ. Codex mới hơn Claude không bị hạ.
