...
- **Phiên cloud đủ plugin như local (02/09/2026, BH86):** `sync/plugin-manifest.json` v2 khai máy «Cloud» + trường `nguon`; hook cloud gọi `tools/cai_plugin_phien_cloud.py --ap-dung` ở nền, cài bằng CLI `claude plugin`; từ chối chạy trên máy thật; mục `chua-ro` chờ `--xuat-nguon` từ Mac. Plugin vẫn chỉ là worker.
- **Đồng bộ skill/plugin tự động:** `sync/skills/` là nguồn duy nhất cho skill riêng. `tools/dong_bo_skill_claude_codex.py` tạo symlink sang `~/.claude/skills` và `~/.codex/skills`, cập nhật runtime Cowork có sao lưu, đối chiếu kho plugin, dựng lại catalog/ZIP `plugin-router-chatgpt` (bỏ qua kèm lời nhắc khi chưa có bản nguồn router trong repo). Sửa nội dung skill có hiệu lực tức thời qua symlink; hook `SessionStart` tự bắt skill mới và thay đổi registry plugin. Không dùng LaunchAgent đọc OneDrive vì macOS TCC chặn tiến trình nền nếu chưa cấp Full Disk Access. Không sửa tay bản trong cache/runtime. Trong bộ hợp nhất, nguồn OneDrive thắng runtime Cowork nhưng bản cũ luôn được sao lưu; khi chạy `dong_bo_skill.py` riêng, mặc định vẫn chặn phân kỳ. Codex mới hơn Claude không bị hạ. **Chạy được trên CẢ HAI máy từ 21/08/2026**: việc tạo liên kết giao cho `tools/lien_ket_da_nen.py` — symlink trên macOS, junction trên Windows (`os.symlink` ném WinError 1314 khi máy chưa bật Developer Mode). Kho plugin đối chiếu bằng `tools/dong_bo_plugin_claude_codex.py` với sổ khai dùng chung `sync/plugin-manifest.json` (khai Ý ĐỊNH — plugin nào cần ở máy nào — nên tách được «thiếu vì cố ý» khỏi «thiếu vì trôi dạt»); mục chưa `da_xac_nhan` chỉ cảnh báo, không báo đỏ. Hook `SessionStart` đi giữa hai máy bằng `tools/dong_bo_hook_sessionstart.py --xuat` / `--ap-dung` — phạm vi `du-an` ghi `.claude/settings.local.json` (không qua git) từ 01/09/2026, vì `.claude/settings.json` nay đi qua git và chỉ khai hook chuẩn bị phiên CLOUD (`.claude/hooks/session-start.sh`: nối skill riêng, khôi phục ngân sách skill từ bản khai chung, thoát ngay khi không phải remote; BH84). Máy có bản `.claude/settings.json` cũ chưa track phải `mv` sang `settings.local.json` một lần trước `git pull`; hook ở hai file cộng dồn. Chốt BH70 canh: không công cụ/cờ nào được trỏ vào khoảng không, Windows không bị chặn, cả hai runtime được nối. **Lối vào duy nhất là `python3 tools/dong_bo_tat_ca.py`** (`--ap-dung` để làm thật, `--liet-ke-lan` xem phạm vi, `--im-khi-on` cho hook; nút bấm đúp ở `sync/dong-bo-tat-ca.command`/`.cmd`): 8 làn theo thứ tự phụ thuộc, làn ① `sync_safety_check` là cổng CHẶN. Chốt BH71 canh lệnh gộp phủ đủ làn và dừng đúng khi nguy hiểm.
...
- Vòng lặp kiểm tra–hoàn thiện toàn hệ tới mức tốt nhất có thể (tự dừng ở human gate): `python tools/system_completion_loop.py --max-iterations 3` *(bí danh tương thích cũ: `tools/clinical_production_loop.py`)*
